...
addMeasurement.get = (req, res) => {
    ...
    // console.log(toTime)
    if (fromTime && !toTime) {
        db.read({table:'measurements', field:'orderguid', fromTime})
            .then(orders => helpers.response(res, 200, orders))
            .catch(err => helpers.response(res, 200, []))
    } else if (fromTime && toTime) {
        // console.log('to and from')
        db.read({table:'measurements', field:'orderguid', fromTime, toTime})
            .then(orders => helpers.response(res, 200, orders))
            .catch(err => helpers.response(res, 200, []))
    } else if (order) {
        db.read({table:'measurements', field:'orderguid', value: order})
            .then(orders => helpers.response(res, 200, orders))
            .catch(err => helpers.response(res, 200, []))
    } else if (scaleId && orderByField) {
        db.read({table:'measurements', field:'scaleid', value: scaleId, orderByField, sort:'desc', limit:50})
            .then(orders => helpers.response(res, 200, orders))
            .catch(err => helpers.response(res, 200, [])) 
    } else if (scaleId) {
        db.read({table:'measurements', field:'scaleid', value: scaleId})
            .then(orders => helpers.response(res, 200, orders))
            .catch(err => helpers.response(res, 200, []))
