...
//Read from DB
lib.read = ({table, field, value, time1, time2, orderByField, sort='asc', limit}) => {
    return new Promise((res, rej) => {
        const con = mysql.createConnection(configDB);
        // range of time
        ...
        if (field && value && time1) {
            ...
            // console.log(time)
            if (!value) {
                con.query(`Select *, s.name as scaleName from ${table} JOIN scales as s ON s.id = ${table}.scaleId WHERE time BETWEEN CAST('${time1}' AS DATE) AND CAST(${time} AS DATE)`, (err, result, fields) => {
